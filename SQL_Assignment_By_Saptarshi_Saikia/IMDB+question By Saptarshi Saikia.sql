...
-- no. of rows in director mapping = 3867
...
-- Type your code below:
select ( select count(*) from movie where id is null) as id ,
...
+---------------+-------------------+ */
-- Type your code below:
select year, count(title) as Total_number_of_movies from movie group by year;
-- Total number of movies released in every year
-- 2017 = 3052
-- 2018 = 2944
-- 2019 = 2001
...
-- March(824) had the most movie releases followed by Septempber(809) and January(804)
...
-- Type your code below:
select count(title) as movies_released_in_India_or_USA , year
from movie 
where (country like '%India%' or country like '%USA%') and year = 2019;

-- Number of movies released in India and USA in the year 2019 is 1059
...
where gen.movie_id = mov.id group by genre order by number_of_movies desc limit 1; 

-- Drama had the most amount of movies produced overall
...
from (select movie_id,count(genre) genre_count
	from genre group by movie_id order by genre_count desc) genre_counts
where genre_count = 1
group by genre_count ;

-- There are 3289 movies listed with only one genre
...
-- Thriller is ranked 3rd with 1484 movies under it
...
-- Dream warrior pictures and National Theatre Live are both tied at first with 3 hit movies each
...
+---------------+-------------------+ */
-- Type your code below:
select genre, count(mov.id) as movie_count from movie as mov
...
-- There are 8 movies that start with 'The' and have an average rating of more than 8.
...
from movie as mov inner join ratings as rat on rat.movie_id = mov.id
...
FROM
    votes_summary;
-- Yes German(106710) movies got more votes than Italian(77965) movies
...
select ( select count(*) from names where id is null) as id ,
...
-- Null values from names are as follows -
-- id = 0
-- name = 0
-- height = 17335
-- date_of_birth = 13431
...
with top_3_genres
as ( select genre, Count(mov.id) AS movie_count , Rank() OVER(ORDER BY Count(mov.id) DESC) AS genre_rank
from movie as mov
inner join genre as gen
on gen.movie_id = mov.id
inner join ratings as rat
...
select nam.name as director_name , Count(dm.movie_id) as movie_count from director_mapping as dm
...
-- Top 3 directors in the top three genres whose movies have an average rating of more than 8 are
-- 1. James Mangold
-- 2. Anthony Russo
-- 3. Soubin Shahir 
...
--  Vijay Sethupathi, Fahadh Faasil and Yogi Babu are the top 3 actors  
...
with actress_detail as( select n.name as actress_name, total_votes,
count(r.movie_id) as movie_count,
...
-- Star Cinema and Twentieth Century Fox have the most number of hits.
